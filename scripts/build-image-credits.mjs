...
const rows = used.map((item) => `<tr><td>${esc(item.title)}</td><td>${esc(item.creator || "Source record")}</td><td><a href="${esc(item.sourceUrl)}" target="_blank" rel="noopener">Wikimedia Commons</a></td><td>${item.licenseUrl ? `<a href="${esc(item.licenseUrl)}" target="_blank" rel="noopener">${esc(item.license)}</a>` : esc(item.license)}</td></tr>`).join("");

const page = `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Photography Credits | Nautical Dream</title>
  <meta name="description" content="Sources, photographers and licenses for documentary photographs published by Nautical Dream.">
...
<main><section class="hero" style="--hero:url('assets/editorial/newport-harbor.jpg')"><div class="shell"><div class="eyebrow">Editorial record</div><h1>Photography credits.</h1><p>Documentary images should have a real place, a real source and traceable permission.</p></div></section>
...
console.log(`Built image-credits.html with ${used.length + 1} credited photographs.`);
